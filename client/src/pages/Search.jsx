...
export default function Search() {
  ...
  const [sidebarData, setSidebarData] = useState({
    ...
    furnished: false,
    offer: false,
    bedrooms: 1, // Added bedrooms
    bathrooms: 1, // Added bathrooms
    sort: 'created_at',
    order: 'desc',
  });
  ...
  useEffect(() => {
    ...
    if (
      ...
      setSidebarData({
        ...
        furnished: furnishedFromUrl === 'true',
        offer: offerFromUrl === 'true',
        bedrooms: parseInt(bedroomsFromUrl) || 1, // Added
        bathrooms: parseInt(bathroomsFromUrl) || 1, // Added
        sort: sortFromUrl || 'created_at',
        order: orderFromUrl || 'desc',
      });
    }
  ...
  useEffect(() => {
    const debounceTimer = setTimeout(() => {
      ...
      urlParams.set('furnished', sidebarData.furnished);
      urlParams.set('offer', sidebarData.offer);
      urlParams.set('bedrooms', sidebarData.bedrooms); // Added
      urlParams.set('bathrooms', sidebarData.bathrooms); // Added
      urlParams.set('sort', sidebarData.sort);
      urlParams.set('order', sidebarData.order);
      const searchQuery = urlParams.toString();
      navigate(`/search?${searchQuery}`, { replace: true });
  ...
  const handleChange = (e) => {
    ...
    if (id === 'all' || id === 'rent' || id === 'sale') {
      ...
      setSidebarData({ ...sidebarData, [id]: parseInt(value) });
    } else if (id === 'sort_order') {
      const [sort, order] = value.split('_');
      setSidebarData({ ...sidebarData, sort: sort || 'created_at', order: order || 'desc' });
    }
